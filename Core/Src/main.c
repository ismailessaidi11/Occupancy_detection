...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
      ...
      err = populateSensorData(&data, &hi2c1, &hrtc, floor_area, room_number, occupancy);
      if (err != NO_ERROR) {
          /* error reading measurement */
    	  ...
    	  if (first_line)
    	  {
    		  if (HAL_UART_Transmit(&huart3, (uint8_t *)COLUMNS, countof(COLUMNS)-1, 1000) != HAL_OK)
    			  Error_Handler();
    		  first_line = 0;
    	  }
    	  print_values(display_str, &data);
...
uint8_t populateSensorData(SensorData *data,  I2C_HandleTypeDef* hi2c, RTC_HandleTypeDef *hrtc, uint8_t floor_area, uint8_t room_number, uint8_t occupancy)
{
...
void print_values(char* display_str, SensorData* data)
{
